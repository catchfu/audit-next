...
export async function getPscStatements(companyNumber, itemsPerPage = 25, startIndex = 0, registerView = false) {
    if (!config.companiesHouseApiKey)
        throw new Error("COMPANIES_HOUSE_API_KEY missing");
    const url = `https://api.company-information.service.gov.uk/company/${companyNumber}/persons-with-significant-control-statements`;
    const params = { items_per_page: itemsPerPage, start_index: startIndex, register_view: registerView };
...
export async function listPsc(companyNumber) {
    if (!config.companiesHouseApiKey)
        throw new Error("COMPANIES_HOUSE_API_KEY missing");
